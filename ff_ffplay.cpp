...
static int audio_decode_frame(FFPlayer *is)
{
    ...
    // 获取声道布局
    dec_channel_layout = (af->frame->channel_layout && af->frame->channels == av_get_channel_layout_nb_channels(af->frame->channel_layout)) ? af->frame->channel_layout : av_get_default_channel_layout(av_frame_get_channels(af->frame));
...
void FFPlayer::check_play_finish()
{
    ...
    if (eof == 1)
    { // 1. av_read_frame已经返回了AVERROR_EOF
...
int FFPlayer::read_thread()
{
    ...
    while (1) {
        ...
        if (seek_req) {
//             /*seek_min 和 seek_max：定义寻址的范围。
// avformat_seek_file：调用函数进行寻址。
// 如果寻址成功，清空音视频队列并发送刷新数据包。
// 通知其他线程寻址完成*/
            // seek_pos 是用户请求的跳转位置
            int64_t seek_target = seek_pos;
            // seek_rel 是相对偏移量，用于调整跳转范围
            // 如果是快进，seek_min 设置为 seek_target - seek_rel + 2，防止跳转位置过小
...
//         /*infinite_buffer：是否允许无限缓冲（通常设置为 0，表示有限缓冲）。
// audioq.size 和 videoq.size：音频和视频队列的当前大小。*/
...
int FFPlayer::video_refresh_thread()
{
    ...
    while (!abort_request)
    {
        /*remaining_time：上次刷新剩余的时间。
av_usleep：让线程休眠指定的微秒数*/
...
void FFPlayer::video_refresh(double *remaining_time)
{
    ...
    retry:
        ...
        else
        {
            ...
            { // 检查当前帧的序列号是否与视频队列的序列号匹配
                frame_queue_next(&pictq);
                goto retry;
            }
            if (lastvp->serial != vp->serial)
            { // 检查上一帧的序列号是否与当前帧的序列号匹配
            ...
            last_duration = vp_duration(lastvp, vp); // 上一帧的持续时间
            //             LOG(INFO) << "last_duration ......" << last_duration;
            delay = compute_target_delay(last_duration); // 计算目标延迟
            ...
            if (!std::isnan(vp->pts))
            {
                update_video_pts(vp->pts, vp->pos, vp->serial); // 更新视频时间戳
            }
            ...
            if (frame_queue_nb_remaining(&pictq) > 1)
            {
                ...
                if (!step && (framedrop > 0 || (framedrop && get_master_sync_type() != AV_SYNC_VIDEO_MASTER)) && time > frame_timer + duration)
                {
                    ...
                    frame_queue_next(&pictq); // 移动到队列中的下一个帧
                    goto retry;
                }
            }
            frame_queue_next(&pictq);
